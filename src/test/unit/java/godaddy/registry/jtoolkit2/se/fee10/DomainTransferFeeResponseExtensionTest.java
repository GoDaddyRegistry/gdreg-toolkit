...
public class DomainTransferFeeResponseExtensionTest {
    ...
    public void shouldGetFeeExtension() throws Exception {
        ...
        final DomainTransferFeeResponseExtension feeResponseExtension =
                new DomainTransferFeeResponseExtension();
        final XMLDocument doc = PARSER.parse(getTransferResponseExpectedXml(dnsForm, true));
        ...
        assertEquals("10.00", feeResponseExtension.getFee("Default").toPlainString());
    ...
    private static String getTransferResponseExpectedXml(final String domainName, boolean feeExtension) {
        ...
        if (feeExtension) {
            result.append("<extension>");
                result.append("<fee:trnData xmlns:fee=\"urn:ietf:params:xml:ns:epp:fee-1.0\">");
